...
impl Keyboard {
   ...
   fn key_hit(&mut self) -> key::Key {
      ...
      // Hier muss Code eingefuegt werden.
       ...
       //busy polling
       while stat ==0 {
           stat = cpu::inb(KBD_CTRL_PORT) & KBD_OUTB;
       }

       // teste, ob data von einer Maus stammt
       let mouse = cpu::inb(KBD_CTRL_PORT) & KBD_AUXB;
       if mouse == 1{
           // data kam von der maus
           return invalid;
       }

       cpu::outb(KBD_CTRL_PORT, KBD_OUTB);
       let data = cpu::inb(KBD_DATA_PORT);
       self.code = data;

       if self.key_decoded() == true &&self.gather.valid() == true{
           return self.gather;
       }else{
           return invalid;
       }
   ...
   fn set_repeat_rate (&mut self, speed: u8, delay: u8) {
      ...
      // Hier muss Code eingefuegt werden.
       cpu::outb(KBD_CTRL_PORT, KBD_CMD_SET_SPEED);
       cpu::outb(KBD_DATA_PORT, (speed << 4)  + (delay <<2));
...
pub fn set_global_repeat_rate(speed: u8, delay: u8){
...
impl Keyboard {
    ...
    *****************************************************************************/
    ...
    fn set_lastkey(&mut self){
        self.lastkey = self.gather.asc;
    }
   ...
   pub fn plugin (&mut self) {
 	   ...
 	   /* Hier muss Code eingefuegt werden */
 	   //im intdispatcher muss die Keyboard ISR an die richtige Stelle geschrieben werden
       //im pic muss per cpu eintrag ein Bit gesetzt werden.
 	   intdispatcher::assign(intdispatcher::INT_VEC_KEYBOARD, Box::new(KeyboardISR));
