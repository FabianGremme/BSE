   ...
   pub fn enqueue(&mut self, data: T) {
    ...
    pub fn remove(&mut self, data: T) -> bool {
       ...
       /* Hier muss Code eingefuegt werden */
        ...
        let Some(m) = prev else { return  false};
        if m.borrow_mut().data == data{
            self.dequeue();
            return true;
        }

        // all cases when it is not the first one to be removed
        let mut node = self.head.clone();
		while let Some(prev) = node {
			let Some(next) = prev.borrow().next.clone() else { return false; };
			
			if  next.borrow_mut().data == data{
				let afternext = next.borrow().next.clone();
				prev.borrow_mut().next = afternext;
				return true;
			}
    ...
    //zeigt, ob sich Inhalte in der queue befinden
