...
pub extern "C" fn int_disp(vector: u32) {
    //cga::print_str("Message from intdispatcher", cga::CGA_STD_ATTR);
...
pub fn assign(vector: usize, callback: Box<dyn isr::ISR>) -> bool {
	...
	// teste ob die Vektornummer innerhalb der Grenze ist
	// dann callback in den Vektor einfügen und 0 ausgeben
	let mut vectors = INT_VECTORS.lock();
	if vector > MAX_VEC_NUM{
	return true;
	}else{
	vectors.map[vector] = callback;
	return false;
	}
}
...
pub fn report(vnum: usize) -> bool {
	...
	/* Hier muss Code eingefuegt werden */
	// hier wird das isr aus der Liste gesucht
	//falls es das standard ding ist, muss false ausgegeben werden
	//ansonsten wird vom isr die trigger aufgerufen und 1 ausgegeben

	let vectors = INT_VECTORS.lock();
	let result = vectors.map.get(vnum);
	match result{
	   Some(result)=>{
	    if result.is_default_isr(){
        	    return false;
        	}else{
        	    result.trigger();
        	    return true;
