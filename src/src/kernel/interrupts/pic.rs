...
pub fn allow (irq: u32) {
 ...
 // irq leitung mithilfe von Registern in der CPU setzen
 //nur in der ersten Maske innerhalb des keyboard das Bit auf 1 setzen

    let mut port = PIC_IMR1;
    let mut num = irq;
    if irq >=8{
       port = PIC_IMR2;
       num = num - 8;
    }
    let value = cpu::inb(port) & !(cpu::inb(port) & (1<<num));
    cpu::outb(port, value);
...
pub fn status (irq: u32) -> bool {
 ...
 /* Hier muss Code eingefuegt werden */
        ...
        let currentval = cpu::inb(port);
        if currentval & (1<<num) > currentval{
            return false;
        }else{
            return true;
        }
